...
opening_hours = json_from_file("../data/sensors/opening_hours.json")
...
def process_sensor(sensor, measures):
    ...
    for i, measure in enumerate(measures):
        ...
        if (values[measure_date.hour] < measure["value"]):
            values[measure_date.hour] = measure["value"]

        # Modifying array of maximums
        if (len(maximums) < 10) or (maximums[0]["nb"] < measure["value"]):
            maximums.append({
                "nb" : measure["value"],
