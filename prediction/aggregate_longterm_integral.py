...
def sum_of_previous(df, row):
    new_row = row[:]
    if (not new_row.name[0]):
        return (0)
    namelist = list(new_row.name)
    namelist[0] -= 1
    new_row.name = tuple(namelist)
    try:
        return (sum_of_previous(df, new_row) + df.loc[new_row.name]["Nb_measured"])
    except:
        return (sum_of_previous(df, new_row))
...
def process_sensor(sensor, measures):
    ...
    integrate = df[:]
    #integrate = integrate.groupby(["Hour_of_day", "Day"]).mean()
    integrate["Last"] = integrate["Nb_measured"].shift(1)
    integrate["Nb_measured"] = integrate.apply(lambda row: row["Nb_measured"] * 6, axis = 1)
    integrate["Tot_measured"] = integrate.apply(lambda row: sum_of_previous(integrate, row), axis = 1)
    integrate["Nb_measured"] = integrate["Tot_measured"][:]
    del integrate["Tot_measured"]
    integrate = integrate.reset_index()
    df = pd.merge(df, integrate, how = "left", on = ["Hour_of_day", "Day"])
    df["Nb_measured"] = df["Nb_measured_y"][:]
    del df["Nb_measured_x"]
    del df["Nb_measured_y"]
    print df
